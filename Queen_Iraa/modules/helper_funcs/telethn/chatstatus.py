from Queen_Iraa.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from Queen_Iraa import DRAGONS
...
async def QUEENIRAA_is_admin(chat_id: int):
    status = False
    QUEENIRAA = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins,
    ):
        if QUEENIRAA.id == user.id:
            status = True
            break
    return status

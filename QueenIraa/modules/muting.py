...
from QueenIraa import GEEZRAM_USERS
...
def check_user(user_id: int, bot: Bot, update: Update) -> Optional[str]:
    ...
    if is_user_admin(update, user_id, member) or user_id in GEEZRAM_USERS:
        return "Can't. Find someone else to mute but not this one."

    return None


@queencmd(command='mute')
@connection_status
@bot_admin
@can_restrict
...
@queencmd(command=['tmute', 'tempmute'])
@connection_status
@bot_admin
@can_restrict
@user_admin(AdminPerms.CAN_RESTRICT_MEMBERS)

from QueenIraa.modules.helper_funcs.telethn import HIGHER_AUTH, telethn
from QueenIraa import SUPPORT_USERS, GEEZRAM_USERS, WHITELIST_USERS
from telethon.tl.types import ChannelParticipantsAdmins


async def user_is_ban_protected(user_id: int, message):
    status = False
    if message.is_private or user_id in (HIGHER_AUTH + SUPPORT_USERS + GEEZRAM_USERS + WHITELIST_USERS):
        return True
...
async def queen_is_admin(chat_id: int):
    status = False
    iraa = await telethn.get_me()
    async for user in telethn.iter_participants(
            chat_id, filter=ChannelParticipantsAdmins):
        if iraa.id == user.id:
            status = True

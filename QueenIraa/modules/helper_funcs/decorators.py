from QueenIraa.modules.disable import DisableAbleCommandHandler, DisableAbleMessageHandler
from telegram.ext import CommandHandler, MessageHandler, CallbackQueryHandler, InlineQueryHandler
from telegram.ext.filters import BaseFilter
from QueenIraa import dispatcher as d, log
from typing import Optional, Union, List


class iraaTelegramHandler:
    def __init__(self, d):
        self._dispatcher = d
    ...
    def message(self, pattern: Optional[BaseFilter] = None, can_disable: bool = True, run_async: bool = True,
        ...
        def _message(func):
            try:
                ...
                log.debug(f"[queenmsg] Loaded filter pattern {pattern} for function {func.__name__} in group {group}")
            except TypeError:
                if can_disable:
                    self._dispatcher.add_handler(
                ...
                log.debug(f"[queenmsg] Loaded filter pattern {pattern} for function {func.__name__}")
    ...
    def inlinequery(self, pattern: Optional[str] = None, run_async: bool = True, pass_user_data: bool = True,
        ...
        def _inlinequery(func):
            ...
            log.debug(
                f'[queeninline] Loaded inlinequery handler with pattern {pattern} for function {func.__name__} | PASSES '
...
queencmd = iraaTelegramHandler(d).command
queenmsg = iraaTelegramHandler(d).message
kigcallback = iraaTelegramHandler(d).callbackquery
queeninline = iraaTelegramHandler(d).inlinequery

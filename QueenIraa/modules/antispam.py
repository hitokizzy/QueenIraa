...
from QueenIraa.modules.sql.users_sql import get_user_com_chats
import QueenIraa.modules.sql.antispam_sql as sql
...
def gban(update: Update, context: CallbackContext):  # sourcery no-metrics
    ...
    if int(user_id) in GEEZRAM_USERS:
        message.reply_text("That's a GEEZRAM! They cannot be banned!")
        return
...
@queenmsg((Filters.all & Filters.chat_type.groups), can_disable=False, group=GBAN_ENFORCE_GROUP)
...
def __user_info__(user_id):
    ...
    if user_id == dispatcher.bot.id:
        return ""
    if int(user_id) in SUDO_USERS + GEEZRAM_USERS + WHITELIST_USERS:
        return ""
    if is_gbanned:

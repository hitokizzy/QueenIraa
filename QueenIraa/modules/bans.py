...
@queencmd(command='ban', pass_args=True)
@connection_status
@bot_admin
@can_restrict
...
def ban(update: Update, context: CallbackContext) -> Optional[str]:  # sourcery no-metrics
    ...
    if is_user_ban_protected(update, user_id, member) and user not in DEV_USERS:
        ...
        elif user_id in GEEZRAM_USERS:
            message.reply_text("Bring an order from Eagle Union to fight a GEEZRAM.")
...
@queencmd(command='tban', pass_args=True)
@connection_status
@bot_admin
@can_restrict
@user_admin(AdminPerms.CAN_RESTRICT_MEMBERS)
...
@queencmd(command='kick', pass_args=True)
@connection_status
@bot_admin
@can_restrict
@user_admin(AdminPerms.CAN_RESTRICT_MEMBERS)
...
@queencmd(command='kickme', pass_args=True, filters=Filters.chat_type.groups)
@bot_admin
@can_restrict
...
@queencmd(command='unban', pass_args=True)
@connection_status
@bot_admin
@can_restrict
@user_admin(AdminPerms.CAN_RESTRICT_MEMBERS)
...
@queencmd(command='selfunban', pass_args=True)
@connection_status
@bot_admin
@can_restrict
@gloggable
def selfunban(context: CallbackContext, update: Update) -> Optional[str]:
    ...
    user = update.effective_user
    bot, args = context.bot, context.args
    if user.id not in SUDO_USERS or user.id not in GEEZRAM_USERS:
        return

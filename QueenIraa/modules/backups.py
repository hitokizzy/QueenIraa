...
from QueenIraa import dispatcher, log as LOGGER, OWNER_ID
from QueenIraa.__main__ import DATA_IMPORT
from QueenIraa.modules.helper_funcs.alternate import typing_action
...
def export_data(update, context):  # sourcery no-metrics
    ...
    with open("iraaRobot{}.json".format(chat_id), "w") as f:
        f.write(str(baccinfo))
    context.bot.sendChatAction(current_chat_id, "upload_document")
    tgl = time.strftime("%H:%M:%S - %d/%m/%Y", time.localtime(time.time()))
    context.bot.sendDocument(
        current_chat_id,
        document=open("iraaRobot{}.json".format(chat_id), "rb"),
        caption="*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `iraaRobot-Backup` was specially made for notes.".format(
    ...
    os.remove("iraaRobot{}.json".format(chat_id))  # Cleaning file

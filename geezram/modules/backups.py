...
def export_data(update, context):
    ...
    with open("geezram{}.backup".format(chat_id), "w") as f:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("geezram{}.backup".format(chat_id), "rb"),
        caption="📤*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `geezram-Backup` was specially made for notes 📚.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("geezram{}.backup".format(chat_id))  # Cleaning file
